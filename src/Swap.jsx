...
function Swap() {
  ...
  style={{
    display: "flex",
    justifyContent: "flex-start",  // Align items from the left
    alignItems: "center",  // Vertically center items
    gap: "40px",  // Add space between the QR code and the text
...
>
  ...
  return (
    <div>
      {/* Show loading animation if loading, else show result */}
      {loading ? (
        <LoadingAnimation />
      ) : resultImageUrl ? (
        <ResultDisplay />
      ) : (
        <div>No result yet.</div> // You can add a message if no result is available
      )}
    </div>
  );
}
