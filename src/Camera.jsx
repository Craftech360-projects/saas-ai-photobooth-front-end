...
function Camer() {
  ...
  const startProcess = () => {
    ...
    setSelectedTheme("superheros");
    setIsThemeSelected(true);
    setImageFolder(`superheros/${userDetails.gender}`);
    setShowScene(true);
  };
    ...
    setTimeout(() => {
      ...
      canvas.toBlob((blob) => {
        ...
        if (section) {
          ...
          setTimeout(() => {
            // Use isImg directly as the key
            const selectedImagePrompt = superheroPrompts[userDetails.gender][isImg];
            console.log("Image path:", isImg);
            console.log("selectedImagePrompt:", selectedImagePrompt);
            navigate("/swap", {
              state: { 
                sourceImage: blob, 
                userDetails, 
